<x-main-layout>
    ...
    <div class="m-2 p-2 flex justify-between">
        ...
        <div class="flex space-x-2">
            From:
            <span class="mx-2">{{ $event->start_date->format('m/d/Y H:m:s') }}</span> | <span
                class="mx-2">{{ $event->end_date->format('m/d/Y H:m:s') }}</span>
        </div>
    </div>
    <div class="mb-16 flex flex-wrap gap-2">
            ...
            <div class="flex flex-col">
                ...
                @auth
                    <div class="flex space-x-2 p-4" x-data="{
                        eventLike: @js($like),
                        onHandleLike() {
                            axios.post(`/events-like/{{ Crypt::encrypt($event->id) }}`).then(res => {
                ...
                <div class="flex flex-col p-4">
                    ...
                    <div class="flex space-x-4 mt-6 bg-slate-200 p-2 rounded-md">
                        ...
                        <div class="flex flex-col">
                            <span class="text-2xl">{{ $event->organizer->name }}</span>
                            <span class="text-2xl">{{ $event->organizer->email }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="w-full shrink-0 grow-0 lg:w-6/12 lg:pl-6 bg-slate-50 rounded-md p-2">
            <p class="mb-6 text-sm text-yellow-600 dark:text-neutral-400">
                Start: <time>{{ $event->start_date->format('d-m-y') }}</time>
            </p>
                ...
                @foreach ($tickets as $ticket)
                    <div class="w-full p-4 duration-500">
                        <div class="flex items-center rounded-lg bg-white p-4 shadow-md shadow-indigo-50">
                            <div>
                                <div class="flex space-x-2">
                                    <h2 class="text-lg font-bold text-gray-900">{{ $ticket->ticket_type_name }}</h2>
                                </div>
                                <p class="mb-6 text-sm text-yellow-600 dark:text-neutral-400">
                                    Start: <time>{{ $event->start_date->format('d-m-y') }}</time>
                                </p>
                                <p class="mb-6 text-sm text-yellow-600 dark:text-neutral-400">
                                    End: <time>{{ $event->end_date->format('d-m-y') }}</time>
                                </p>
                                {{-- <p class="text-sm font-semibold text-gray-400">Qty: {{ $ticket->quantity }}</p> --}}
                                {{-- @can('view', $ticket) --}}
                                    {{-- <form action="{{ route('events.comments.destroy', [$event->id, $ticket->id]) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button
                                            class="mt-6 rounded-lg bg-slate-400 px-4 py-2 text-sm tracking-wider text-white outline-none hover:bg-slate-300">Buy</button>
                                    ...
                                    <form action="{{ route('tickets.purchase', $ticket->id) }}" method="POST">
                                        ...
                                        <!-- Quantity input -->
                                        <div class="flex space-x-2">
                                            <label for="quantity" class="mb-6 text-sm text-yellow-600 dark:text-neutral-400">Quantity:</label>
                                            <input type="number" id="quantity" name="quantity" x-model.number="quantity" @input="if (quantity > maxQuantity) quantity = maxQuantity; if (quantity < 1) quantity = 1;"
                                            min="1"
                                            :max="maxQuantity"
                                        ...
                                        <div>
                                            <p class="mb-6 text-sm text-yellow-600 dark:text-neutral-400">Price per ticket: ${{ $ticket->price }}</p>
                                            <p class="mb-6 text-sm text-yellow-600 dark:text-neutral-400">Total: $<span x-text="total()"></span></p>
                                        </div>
                                        @if($event->organizer_id != Auth::user()?->id)
                                            ...
                                            class="mt-6 rounded-lg bg-slate-400 px-4 py-2 text-sm tracking-wider text-white outline-none hover:bg-slate-300">Buy</button>
                                            @else
                                            <p class="mb-6 text-sm text-yellow-600 dark:text-neutral-400">Sold Out</p>
                                            @endif
                                        @endif
                                        </form>
                                {{-- @endcan --}}
            ...
            @auth
                <div
                    ...
                    <div class="">
                        <form action="{{ route('events.comments', Crypt::encrypt($event->id)) }}" class="flex justify-between space-x-2"
                            method="POST">
                            @csrf
                            <input type="text"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
